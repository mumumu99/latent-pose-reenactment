...
IDENTITIES=("$1") # or `(*/*)`, or whatever else
cd -
    ...
    for IDENTITY in "${IDENTITIES[@]}"; do #$DATASET_ROOT/images/*; do
        if (($i >= $FIRST_IDX && $i < $LAST_IDX)); then
            ...
            if [ "$DO_COMPUTE_LANDMARKS" = true ]; then
                ...
                python3 /content/drive/MyDrive/latent-pose-reenactment/utils/crop_as_in_dataset.py --crop-style=latentpose "$DATASET_ROOT/images/$IDENTITY/" "$DATASET_ROOT/$IMAGES_CROPPED_DIR_NAME/$IDENTITY/"
            fi
        fi
        let "i = i + 1"
    done;
...
if [ "$DO_COMPUTE_SEGMENTATION" = true ]; then
    ...
    cd /content/drive/MyDrive/latent-pose-reenactment/utils/Graphonomy
